...
export const getJourneyViewportHeight = () => Math.max(window.innerHeight, 1);
...
const interpolateLook = (
  ...
  fadeCol: mixArray(a.fadeCol!, b.fadeCol!, t),
});

export function useJourneyDirector(
  blobRef: RefObject<BelisBlobHandle | null>,
  locked: boolean,
) {
  ...
  useEffect(() => {
    let raf = 0,
      displayed = clamp(
        window.scrollY / getJourneyViewportHeight(),
        0,
        STATION_COUNT - 1,
      ),
      frame = 0;
    ...
    const step = () => {
      ...
      const i = Math.min(Math.floor(p), STATION_COUNT - 2),
        fraction = p - i;
      ...
      if (frame++ % 6 === 0) {
        ...
        setHud((value) => ({
          ...value,
          progress: p / (STATION_COUNT - 1),
          active,
          act: acts[active],
          rotation: Math.round(blobRef.current?.getRotationDeg() ?? 0),
        }));
      }
